...
`).map(e=>e.replace(/\r$/,``));if(r.length<2)return c=!0,o.movies;let i=p(r[0]),a={};i.forEach((e,t)=>{a[e.trim()]=t});let l=Object.keys(e),h=0;for(let e=1;e<r.length;e++){if(!r[e].trim())continue;let t=p(r[e]),n=m(t[a.Title]);if(!n)continue;h++;let i={id:h,title:n,titleJP:m(t[a[`Title (Japanese)`]])||``,year:m(t[a.Year])||``,imdbScore:m(t[a[`IMDB Score`]])||m(t[a.IMDB])||``,rottenScore:m(t[a[`Rotten Tomato`]])||m(t[a.Rotten])||``,posterUrl:null};o.movies.push(i),s.movieById.set(h,i),u(h,n),i.titleJP&&u(h,i.titleJP);for(let e of l){let n=a[`${e} Video`],r=a[`${e} Audio`];if(n===void 0)continue;let i=m(t[n])||`-`,c=m(t[r])||`-`;if(i===`-`&&c===`-`)continue;let l=s.countryByCode.get(e);if(!l)continue;let u=o.qualities.length,p={movieId:h,countryId:l.id,videoFormat:i,audioFormat:c};o.qualities.push(p),d(s.videoFormat,i,u),d(s.audioFormat,c,u),f(s.movieQualities,h,u),f(s.countryQualities,l.id,u)}}return c=!0,console.log(`DB loaded: ${o.movies.length} movies, ${o.countries.length} countries, ${o.qualities.length} quality records`),o.movies}function g(e,t){if(!e||!e.trim())return[];let n=e.toLowerCase().trim(),r=new Map;function i(e,t){r.set(e,(r.get(e)||0)+t)}for(let e of o.movies){let t=e.title.toLowerCase();t===n?i(e.id,1e3):t.startsWith(n)&&i(e.id,500)}if(n.length>=3){let e=[];for(let t=0;t<=n.length-3;t++){let r=n.substring(t,t+3),i=s.titleTrigrams.get(r);i&&e.push(i)}if(e.length>0){let t=new Map;for(let n of e)for(let e of n)t.set(e,(t.get(e)||0)+1);for(let[n,r]of t){let t=r/e.length;t>=.5&&i(n,t*200)}}}let a=n.split(/\s+/).filter(e=>e.length>=2);for(let e of a){let t=s.titleWords.get(e);if(t)for(let e of t)i(e,100);if(e.length>=3)for(let t=0;t<=e.length-3;t++){let n=e.substring(t,t+3),r=s.titleTrigrams.get(n);if(r)for(let e of r)i(e,10)}}for(let e of o.movies)e.titleJP&&e.titleJP.includes(n)&&i(e.id,300);return[...r.entries()].sort((e,t)=>t[1]-e[1]).slice(0,50).map(([e])=>s.movieById.get(e)).filter(Boolean)}function _(e){let t=s.movieQualities.get(e)||[],n={};for(let e of t){let t=o.qualities[e],r=o.countries.find(e=>e.id===t.countryId);r&&(n[r.countryCode]={video:t.videoFormat,audio:t.audioFormat})}return n}var v=`bda8d46b3b1b9c717d1d3448901c2a93`,y=`https://api.themoviedb.org/3`,b=`https://image.tmdb.org/t/p`,x=`tmdb_poster_cache`,S=`tmdb_poster_cache_v`,C=2,w=T();function T(){try{return parseInt(localStorage.getItem(S)||`0`)<C?(localStorage.removeItem(x),localStorage.setItem(S,String(C)),{}):JSON.parse(localStorage.getItem(x)||`{}`)}catch{return{}}}function E(){try{let e=Object.keys(w);e.length>3e3&&e.slice(0,e.length-2e3).forEach(e=>delete w[e]),localStorage.setItem(x,JSON.stringify(w))}catch{}}async function D(e,t,n=`w342`){let r=`${e}_${t}`;if(w[r]!==void 0)return w[r]?`${b}/${n}${w[r]}`:null;try{let i=new URLSearchParams({api_key:v,query:e,language:`en-US`});t&&i.set(`year`,t);let a=await(await fetch(`${y}/search/movie?${i}`)).json();if(a.results&&a.results.length>0){let t=e.toLowerCase(),i=(a.results.find(e=>e.title&&e.title.toLowerCase()===t)||a.results[0]).poster_path;return w[r]=i||null,E(),i?`${b}/${n}${i}`:null}return w[r]=null,E(),null}catch(t){return console.warn(`TMDb error for "${e}":`,t),null}}var O={"Dolby Vision":4,"HDR10+":3,HDR10:3,HDR:3,SDR:1,"-":0,"":0},k={Atmos:3,"Dolby Atmos":3,"5.1":2,Stereo:1,"-":0,"":0};function A(e){return O[e]??0}function j(e){return k[e]??0}function M(e,t){return A(e)*10+j(t)}function N(e,t){if(!e||e===`-`)return`Not Available`;let n=[];return e===`Dolby Vision`||e===`HDR10`||e===`HDR10+`||e===`HDR`?n.push(`4K ${e}`):e===`SDR`?n.push(`HD SDR`):n.push(e),t&&t!==`-`&&n.push(t===`Atmos`?`Dolby Atmos`:t),n.join(` + `)}function P(e){let t={},n=-1,r=``,i=0,a=0,o=0,s=_(e.id);for(let[e,c]of Object.entries(s)){if(!c.video||c.video===`-`)continue;let s=N(c.video,c.audio),l=M(c.video,c.audio);t[s]||(t[s]={label:s,score:l,countries:[]}),t[s].countries.push(e),l>n&&(n=l,r=s),(c.audio===`Atmos`||c.audio===`Dolby Atmos`)&&i++,c.video===`Dolby Vision`&&a++,c.video!==`SDR`&&c.video!==`-`&&o++}let c=Object.values(t).sort((e,t)=>t.score-e.score);return{bestLabel:r,bestScore:n,bestCountries:t[r]?.countries||[],groups:c,atmosCount:i,dvCount:a,fourKCount:o}}var F=[],I=JSON.parse(localStorage.getItem(`watchlist`)||`[]`);function L(){let e=(window.location.hash||`#/`).slice(2).split(`/`);return{page:e[0]||`home`,param:e[1]||``}}function R(e){window.location.hash=`#/`+e}var z=()=>document.getElementById(`page-content`);function B(){let e=L();document.querySelectorAll(`.nav-link`).forEach(t=>{let n=t.dataset.page;t.classList.toggle(`active`,n===e.page||e.page===`search`&&n===`home`)})}async function V(){let e=L();B();let t=z();switch(t.innerHTML=``,t.className=`page-enter`,e.page){case`home`:U(t);break;case`search`:W(t,e.param);break;case`movie`:G(t,e.param);break;case`discover`:K(t);break;case`analytics`:q(t);break;case`watchlist`:J(t);break;default:U(t)}}function H(e){let t=P(e),n=document.createElement(`a`);n.className=`poster-card`,n.href=`#/movie/${e.id}`;let r=document.createElement(`div`);if(r.className=`poster-image-wrap`,e.poster){let t=document.createElement(`img`);t.className=`poster-image`,t.src=e.poster,t.alt=e.title,t.loading=`lazy`,r.appendChild(t)}else{let t=document.createElement(`div`);t.className=`poster-no-image`,t.textContent=`🎬`,r.appendChild(t),D(e.title,e.year).then(t=>{if(t){e.poster=t;let n=document.createElement(`img`);n.className=`poster-image`,n.src=t,n.alt=e.title,r.innerHTML=``,r.appendChild(n)}})}let i=document.createElement(`div`);i.className=`poster-badges`,t.dvCount>0&&(i.innerHTML+=`<span class="badge badge-dv">DV</span>`),t.atmosCount>0?i.innerHTML+=`<span class="badge badge-atmos">Atmos</span>`:t.fourKCount>0&&(i.innerHTML+=`<span class="badge badge-4k">4K</span>`),r.appendChild(i),n.appendChild(r);let a=document.createElement(`div`);return a.className=`poster-info`,a.innerHTML=`
  ...
  `;let t=document.getElementById(`home-search`);t.addEventListener(`keydown`,e=>{if(e.key===`Enter`){let e=t.value.trim();e&&(R(`search/${encodeURIComponent(e)}`),requestAnimationFrame(()=>{let e=document.getElementById(`search-input`);e&&(e.focus(),e.selectionStart=e.selectionEnd=e.value.length)}))}});let n=F.filter(e=>P(e).dvCount>0&&e.imdbScore).sort((e,t)=>parseFloat(t.imdbScore||0)-parseFloat(e.imdbScore||0)).slice(0,20),r=document.getElementById(`trending-carousel`);n.forEach(e=>r.appendChild(H(e)));let i=F.filter(e=>{let t=P(e);return t.dvCount>0&&t.atmosCount>0}).sort((e,t)=>parseFloat(t.imdbScore||0)-parseFloat(e.imdbScore||0)).slice(0,24),a=document.getElementById(`dv-atmos-grid`);i.forEach(e=>a.appendChild(H(e)))}function W(e,t){let n=decodeURIComponent(t),r=g(n,F);e.innerHTML=`
    <div class="search-container">
      <span class="search-icon">🔍</span>
      <input type="text" class="search-input" id="search-input" value="${X(n)}" placeholder="Search movies..." autocomplete="off" />
    </div>
    <h2 class="section-title" id="search-count">${r.length} results for "${X(n)}"</h2>
    <div class="poster-grid" id="search-grid"></div>
  `;let i=document.getElementById(`search-input`),a;function o(){let e=i.value.trim();if(e.length<2)return;let t=g(e,F),n=`#/search/${encodeURIComponent(e)}`;history.replaceState(null,``,n);let r=document.getElementById(`search-count`);r&&(r.textContent=`${t.length} results for "${e}"`);let a=document.getElementById(`search-grid`);a&&(a.innerHTML=``,t.forEach(e=>a.appendChild(H(e))))}i.addEventListener(`input`,()=>{clearTimeout(a),a=setTimeout(o,400)}),i.addEventListener(`keydown`,e=>{e.key===`Enter`&&(clearTimeout(a),o())}),requestAnimationFrame(()=>{i.focus(),i.selectionStart=i.selectionEnd=i.value.length});let s=document.getElementById(`search-grid`);r.forEach(e=>s.appendChild(H(e)))}function G(e,t){let n=F.find(e=>e.id===parseInt(t));if(!n){e.innerHTML=`<p>Movie not found</p>`;return}let r=P(n),i=I.includes(n.id);if(e.innerHTML=`
    ...
    <div class="movie-detail">
      <div class="movie-hero">
        ...
        <div class="movie-hero-info">
          <h1 class="movie-title-main">${X(n.title)}</h1>
          ${n.titleJP?`<p style="color:var(--text-secondary);font-size:0.95rem">${X(n.titleJP)}</p>`:``}
  ...
  `,document.getElementById(`back-btn`).onclick=()=>history.back(),D(n.title,n.year,`w500`).then(e=>{let t=document.getElementById(`hero-poster`);e&&(t.innerHTML=`<img src="${e}" alt="${X(n.title)}" style="width:100%;aspect-ratio:2/3;object-fit:cover" />`)}),r.bestCountries.length>0){let e=document.getElementById(`best-flags`);r.bestCountries.forEach(t=>{e.appendChild(a(t,!0,n.title))})}let o=document.getElementById(`quality-groups`);r.groups.forEach(e=>{let t=document.createElement(`div`);t.className=`quality-group`,t.innerHTML=`<div class="quality-group-label">${e.label} <span style="color:var(--text-muted);font-size:0.8rem">(${e.countries.length} countries)</span></div>`;let r=document.createElement(`div`);r.className=`flag-row`,e.countries.forEach(e=>{r.appendChild(a(e,!0,n.title))}),t.appendChild(r),o.appendChild(t)}),document.getElementById(`watchlist-btn`).onclick=()=>{Y(n.id),G(e,t)}}function K(e){e.innerHTML=`
  ...
  `;let d=document.getElementById(`ranking-list`);u.forEach((e,t)=>{let r=document.createElement(`div`);r.className=`ranking-item`,r.innerHTML=`
    ...
    `,d.appendChild(r)})}function J(e){let t=F.filter(e=>I.includes(e.id)),n=0,r=0;if(t.forEach(e=>{let t=P(e);t.atmosCount>0&&n++,t.dvCount>0&&r++}),e.innerHTML=`
    ...
    `}
  `,t.length>0){let e=document.getElementById(`watchlist-grid`);t.forEach(t=>e.appendChild(H(t)))}}function Y(e){let t=I.indexOf(e);t>=0?I.splice(t,1):I.push(e),localStorage.setItem(`watchlist`,JSON.stringify(I))}function X(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}async function Z(){window.addEventListener(`hashchange`,V);try{F=await h()}catch(e){console.error(`Failed to load movies:`,e)}V()}Z();